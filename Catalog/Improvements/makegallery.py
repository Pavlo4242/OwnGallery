...
class MediaGalleryHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # --- API ENDPOINT: /api/root_dir ---
        if path == '/api/root_dir':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'root_dir': DIRECTORY}).encode())
            return
        ...
        if path == '/api/config':
            ...
            config_data = self.scan_for_json_configs()
    ...
    def scan_for_json_configs(self):
        """Scan directory for JSON files and extract configuration data"""
        config_data = {
            "available_configs": [],
            "current_config": None,
            "fields": {}
        ...
        for json_file in json_files:
            try:
                ...
                config_info = {
                    "filename": json_file,
                    "display_name": data.get("config_name", json_file),
                    "description": data.get("description", ""),
                    "fields": self.extract_fields(data)
                }
                
                config_data["available_configs"].append(config_info)
                
                # Use the first JSON as current config, or look for a specific one
                if config_data["current_config"] is None:
                    config_data["current_config"] = config_info
                    config_data["fields"] = config_info["fields"]
    ...
    def extract_fields(self, json_data):
        """Extract form fields from JSON structure"""
        fields = {}
        ...
        def traverse(obj, path=""):
            if isinstance(obj, dict):
                for key, value in obj.items():
                    ...
                    # Check if this is a field we should make editable
                    if self.is_editable_field(key, value):
                ...
                # Check first element to see if it's a simple list
    ...
    def is_editable_field(self, key, value):
        """Determine if a field should be made editable in the UI"""
        # Skip metadata fields and very long values
        skip_keys = {'config_name', 'description', 'timestamp', 'id', '_id'}
        if key in skip_keys:
            return False
            
        # Only allow simple types for editing
    ...
    def determine_field_type(self, value):
        """Determine the HTML input type based on value"""
